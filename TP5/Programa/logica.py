    ...
    while simulaciones[-1].reloj <= cantidad_tiempo:
        ...
        if i == 100000:
            ...
            # Si llega el técnico
                ...
                terminales_libres = simulaciones[i].get_terminales_libres()
            ...
            # Si se termina de registrar una huella en una terminal
                ...
                numero_terminal_fin_registro_actual = int(evento[-2])

                # Destruir el empleado que terminó de registrar su huella
                numero_empleado_a_eliminar = simulaciones[i].buscar_empleado(numero_terminal_fin_registro_actual)
                ...
                asignacion_tecnico = False
                asignar_empleado = False

                # Ver si el técnico está esperando, y asignarselo a la terminal si todavia no fue mantenida
                if simulaciones[i - 1].estado_tecnico == "EFR":
                    ...
                    if simulaciones[i - 1].mantenimiento_t1 == "NO" and numero_terminal_fin_registro_actual == 1:
                        ...
                        simulaciones[i].fin_registro_huella_t2 = simulaciones[i - 1].fin_registro_huella_t2
                        simulaciones[i].fin_registro_huella_t3 = simulaciones[i - 1].fin_registro_huella_t3
                        simulaciones[i].terminales[numero_terminal_fin_registro_actual - 1].estado = "OM"
                    ...
                    # En caso que no encuentre ninguna por mantener
                    if not asignacion_tecnico:
                        simulaciones[i].estado_tecnico = "EFR"
                        asignar_empleado = True
                ...
                # En cambio, si el tecnico está ocupado o libre, no tocar nada respecto al mantenimiento
                if simulaciones[i-1].estado_tecnico != "EFR" or asignar_empleado:
                    # Si tenemos empleados esperando en cola
                    if simulaciones[i - 1].get_cola() > 0:
                        ...
                        estado_empleado = "HR" + '(' + str(numero_terminal_fin_registro_actual) + ')'
                        ...
                        simulaciones[i].empleados[indice_empleado].terminal = numero_terminal_fin_registro_actual

                        simulaciones[i].terminales[numero_terminal_fin_registro_actual - 1].estado = simulaciones[i - 1].terminales[numero_terminal_fin_registro_actual - 1].estado

                        if numero_terminal_fin_registro_actual == 1:
                        ...
                        if numero_terminal_fin_registro_actual == 2:
                        ...
                        if numero_terminal_fin_registro_actual == 3:
                        ...
                        if numero_terminal_fin_registro_actual == 4:
                    ...
                    # Si ya no tenemos empleados esperando en cola
                    else:
                        simulaciones[i].cola = simulaciones[i - 1].cola
                        simulaciones[i].terminales[numero_terminal_fin_registro_actual-1].estado = "L"
                ...
                simulaciones[
                    i].acumulador_empleados_que_pasaron_por_el_sistema = acumulador_empleados_que_pasan_por_sistema
                simulaciones[i].acumulador_empleados_que_salen_temporalmente = simulaciones[
                    i - 1].acumulador_empleados_que_salen_temporalmente
                simulaciones[i].proxima_llegada_empleado = simulaciones[i - 1].proxima_llegada_empleado
                simulaciones[i].proxima_llegada_tecnico = simulaciones[i - 1].proxima_llegada_tecnico
            ...
            # Si el tecnico termina de hacer mantenimiento a una terminal
                ...
                numero_terminal_fin_mantenimiento_actual = int(evento[-2])

                if numero_terminal_fin_mantenimiento_actual == 1:
                    simulaciones[i].mantenimiento_t1 = "SI"
                    simulaciones[i].mantenimiento_t2 = simulaciones[i - 1].mantenimiento_t2
                    simulaciones[i].mantenimiento_t3 = simulaciones[i - 1].mantenimiento_t3
                    simulaciones[i].mantenimiento_t4 = simulaciones[i - 1].mantenimiento_t4

                elif numero_terminal_fin_mantenimiento_actual == 2:
                    simulaciones[i].mantenimiento_t2 = "SI"
                    simulaciones[i].mantenimiento_t1 = simulaciones[i - 1].mantenimiento_t1
                    simulaciones[i].mantenimiento_t3 = simulaciones[i - 1].mantenimiento_t3
                    simulaciones[i].mantenimiento_t4 = simulaciones[i - 1].mantenimiento_t4

                elif numero_terminal_fin_mantenimiento_actual == 3:
                    simulaciones[i].mantenimiento_t3 = "SI"
                    simulaciones[i].mantenimiento_t2 = simulaciones[i - 1].mantenimiento_t2
                    simulaciones[i].mantenimiento_t1 = simulaciones[i - 1].mantenimiento_t1
                    simulaciones[i].mantenimiento_t4 = simulaciones[i - 1].mantenimiento_t4

                elif numero_terminal_fin_mantenimiento_actual == 4:
                    simulaciones[i].mantenimiento_t4 = "SI"
                    simulaciones[i].mantenimiento_t2 = simulaciones[i - 1].mantenimiento_t2
                    simulaciones[i].mantenimiento_t3 = simulaciones[i - 1].mantenimiento_t3
                    simulaciones[i].mantenimiento_t1 = simulaciones[i - 1].mantenimiento_t1

                proxima_terminal = simulaciones[i-1].get_terminal_a_mantener(numero_terminal_fin_mantenimiento_actual)

                # Si todas las terminales están ocupadas, pero quedan restantes por mantener
                if not proxima_terminal:
                    simulaciones[i].estado_tecnico = "EFR"
                    simulaciones[i].cola = simulaciones[i - 1].cola
                    simulaciones[i].terminales[int(proxima_terminal) - 1].estado = simulaciones[i - 1].terminales[
                        int(proxima_terminal) - 1].estado

                # Si no hay mas terminales por mantener, se lanza evento llegada_tecnico de vuelta
                elif proxima_terminal == -1:
                    ...
                    simulaciones[i].tiempo_en_llegar_tecnico = generador_uniforme(a=dist_tecnico.get_a(),
                                                                                  b=dist_tecnico.get_b(),
                                                                                  random=simulaciones[
                                                                                      i].rnd_llegada_tecnico)

                    simulaciones[i].proxima_llegada_tecnico = round(
                        simulaciones[i].reloj + simulaciones[i].tiempo_en_llegar_tecnico, 2)
                    ...
                    simulaciones[i].estado_tecnico = "L"
                ...
                if simulaciones[i - 1].get_cola() > 0:
                    simulaciones[i].terminales[numero_terminal_fin_mantenimiento_actual - 1].estado = "OR"
                    ...
                    # Cambiar el estado de ese empleado de EC a HR
                    estado_empleado = "HR" + '(' + str(numero_terminal_fin_mantenimiento_actual) + ')'
                    ...
                    simulaciones[i].empleados[indice_empleado].numero = num_empleado
                    simulaciones[i].empleados[indice_empleado].minuto_entrada_cola = ""
                    simulaciones[i].empleados[indice_empleado].terminal = numero_terminal_fin_mantenimiento_actual
                    ...
                    if numero_terminal_fin_mantenimiento_actual == 2:
                        simulaciones[i].fin_registro_huella_t2 = round(
                            reloj + simulaciones[i].tiempo_registro_huella, 2)
                        simulaciones[i].fin_registro_huella_t1 = simulaciones[i - 1].fin_registro_huella_t1
                        simulaciones[i].fin_registro_huella_t3 = simulaciones[i - 1].fin_registro_huella_t3
                    ...
                    if numero_terminal_fin_mantenimiento_actual == 3:
                        simulaciones[i].fin_registro_huella_t3 = round(
                            reloj + simulaciones[i].tiempo_registro_huella, 2)
                        simulaciones[i].fin_registro_huella_t2 = simulaciones[i - 1].fin_registro_huella_t2
                        simulaciones[i].fin_registro_huella_t1 = simulaciones[i - 1].fin_registro_huella_t1
                    ...
                    if numero_terminal_fin_mantenimiento_actual == 4:
                        simulaciones[i].fin_registro_huella_t4 = round(
                            reloj + simulaciones[i].tiempo_registro_huella, 2)
                        simulaciones[i].fin_registro_huella_t2 = simulaciones[i - 1].fin_registro_huella_t2
                        simulaciones[i].fin_registro_huella_t3 = simulaciones[i - 1].fin_registro_huella_t3
                    ...
                    simulaciones[i].cola = simulaciones[i - 1].cola
                    simulaciones[i].acumulador_tiempo_espera = simulaciones[i - 1].acumulador_tiempo_espera
                    simulaciones[i].terminales[numero_terminal_fin_mantenimiento_actual - 1].estado = "L"
